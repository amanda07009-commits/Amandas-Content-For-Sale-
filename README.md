# Tiger Studios: portfolio site

Photo and film content shoots for lifestyle businesses across Southeast Asia, by Amanda Marsh.
...
|---|---|
| Business name, your name, Instagram, WhatsApp, email | `business`, `name`, `instagram`, `whatsapp`, `email` |
...
All colours and fonts are in the **BRAND TOKENS** block at the top of the `<style>`: black, cream and tiger orange from the logo, with DM Serif Display, Archivo Black and DM Sans. Swap those values to restyle the whole site. The logo drafts are in `logos/`. `--grain` controls the film grain (set it to `0` to turn it off).
...
1. Buy the domain at Cloudflare Registrar: a `.com` is about $10–12 a year; endings like `.studio` usually cost more.
...
4. Update `og:image` in the `<head>` of `index.html` to the full address, e.g. `https://tigerstudios.studio/photos/share.jpg`.
